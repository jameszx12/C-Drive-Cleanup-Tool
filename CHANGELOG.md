...
## v4.3.1（2026-09-13）响应式布局修复

### 用户反馈
"改变横宽占比后排布会乱"

### 根因（四处叠加，逐条实测确认）
- **grid 列 `uniform` 残留（主因）**：`_configure_columns` 缩列时只把多余列 `weight=0`，没清 `uniform`。残留的 uniform 组仍让空列参与等宽分配 —— 容器 912px 下第 2 列停用后仍占 **357px**，两张可见卡被挤到 277px，右侧空出一整列
- **卡片高度随内容漂移**：`_add_row` 用 `grid_propagate(False)`，但卡片子控件是 pack 布局，该调用对 pack 子控件无效 → 卡片高度由描述换行数决定，同一行卡片高矮参差。应为 `pack_propagate(False)`
- **扫描全程不重排列数**：`_on_list_configure` 开头 `if not self.has_scanned: return`，而 `has_scanned` 在整个扫描期间都是 False → 扫描中改变窗口宽度完全不响应、不重排；首扫建卡也用不上当前的真实宽度
- **卡片描边停留在旧尺寸**：`_schedule_shell_draw` 只响应前 3 次 `<Configure>`，启动期用完后，拉伸窗口时描边/高光/阴影永远停在旧尺寸（卡片边线断开、高光短线）

### 修复
- `_configure_columns`：缩列时同时 `uniform=""`，空列彻底不占宽
- `_add_row`：改 `pack_propagate(False)`；卡片高度与骨架屏统一为 `widgets.CARD_H`（骨架→真实卡片不再纵向跳动）
- 移除 `has_scanned` 早退，抽出 `_sync_cols()`；加两道前置判断（窗口未映射 / 宽度退化到单列以下）避免被布局中间态（实测 250、528）污染列数
- `_schedule_shell_draw`：改为 `after_idle` 防抖 + 尺寸未变直接跳过，拖拽窗口时描边始终贴合
- 卡片间距 `_add_row`(4,4) 与 `_relayout_cards`(6,5) 不一致 → 统一为 `CARD_GAP_X/Y`
- 描述换行宽度按 24px 量化 + 值未变跳过，减少拖拽时的文本重排抖动

### 同时修复（审计发现）
- **工具栏指标卡溢出**：宽度不足时 pack 会把统计卡压扁或挤出边界（1024px 下第一张被压到 64px、第二张越界裁切）→ 按剩余宽度三级降级（两张 / 一张 / 隐藏），1000–1650px 实测均不越界
- **搜索框内 Ctrl+A 误全选清理项**：Tk 在 Windows 上未给 Entry 绑 Ctrl+A，窗口级绑定接管 → 在搜索框按 Ctrl+A 会把所有清理项勾上，而框内文本毫无反应。现在输入框内改为全选文本
- **清理中快捷键仍能改勾选**：`_set_all` 缺 `cleaning` 守卫（`toggle_sel` 有，不一致）
- **选中描边在尺寸变化后被重置**：shell 重绘用默认描边色覆盖选中态 → 记录 `_current_border` 后重绘沿用
- 扫描 / 清理中切换分类由静默无响应改为 toast 提示
- 侧边栏选中竖条的首次绘制早于控件映射导致不显示；空列表时「全选」按钮图标尺寸与常态不一致（20 vs 17）
- 主题切换重建界面时复位 `_toolbar_w` / `_toolbar_stats_mode` / `_last_wrap`，否则同尺寸窗口下工具栏降级判断被跳过、窄窗口下统计卡溢出

### 验证
- 新增 `tests/test_layout_resize.py`：真实映射窗口 + 宽度变化，断言「停用列不占宽 / 同行卡片等高 / 描边贴合当前尺寸 / 选中描边保持 / 未扫描时列数也跟随宽度」
- 真实扫描 15,510 个文件 / 23 张卡，在 1024 / 1180 / 1300 / 1500 / 1650px、深色 + 浅色、以及反复切换主题后复核：列数随宽度在 3 列与 2 列间正确切换、停用列占宽 0、同行行高一致、工具栏不越界、扫描结束后按钮恢复实心色
- `selftest` / `test_regress` / `test_perf_features` / `smoke_gui_test` / `test_layout_resize` 全部通过

## v4.3.0（2026-09-12）Bug 猎杀 + 精致化

### Bug 修复（实测/审计发现）
- **particles 性能测试模块已崩**：`config` v4.0 换肤后缺 `C_BG_TOP/C_BG_MID/C_BG_BOT`，任何实例化 `AnimatedBackground`（含 `tests/test_perf_features.py`）必 `AttributeError` → 补渐变三段别名
- **扫描中输入搜索会吞掉列表（必现）**：`_apply_visible_rules` 把欢迎层盖到骨架屏/卡片上，扫完后列表仍藏在后面 → 忙时禁用搜索框 + `_do_search_change` 兜底直接返回
- **Tooltip 注册表泄漏**：只写不读的 `_registry` 强引用已销毁控件，跨扫描/跨主题切换越积越多 → 移除，生命周期跟随宿主
- **标题栏**：去掉标题里的作者后缀（更干净），源码运行也能显示任务栏/标题图标（`iconbitmap`，exe 本就有）；去掉状态栏 `🙂` 表情（跨设备渲染不一致显廉价）

### 精致化（截屏逐帧验收，深/浅双主题）
- **实心主按钮**（变化最大）：`GlassButton` 新增 `solid` 模式——近实心强调色 + 按亮度自动文字色（蓝底白字/琥珀底深字），终结原来 30% 混合在深色底上发灰、琥珀混成"橄榄泥"的廉价感；工具栏扫描/清理、欢迎页 CTA、对话框关闭/保存已切换
- **对比度系统性拉起**：图标胶囊底（头部/统计卡/步骤卡/清理卡）加深一档；详情对话框类型/版本徽章改白字+加深底；风险项"需确认"由纯色文字改为语义徽章
- README 版本号同步到 v4.3.0（之前停留在 v4.1.0）

### 验证
- 深/浅色欢迎页、扫描列表、文件夹详情对话框截屏验收
- `selftest` / `test_regress` / `smoke_gui_test` 全部通过

## v4.2.1（2026-09-12）扫描结果排序稳定化

### 用户反馈
"怎么每次扫描后的待清理项的排序都不同"

### 根因
- v4.2 改为多核并行扫描后，卡片按"谁先扫完谁先建卡"的完成顺序展示，而完成顺序每次随机 → 每次扫描卡片排序都不同
- v4.2 之前是串行扫描，天然按 `rules` 定义顺序建卡，所以排序稳定

### 修复
- `start_scan()` 记录规则原始顺序 `_rule_order`；`_finish_scan()` 收尾时（完成/取消两条路径都）调用新增 `_sort_cards_by_rule_order()` 按原始顺序重排一次（复用 `_relayout_cards` 做 grid 重排）
- 扫描中仍渐进展示（快），结束时一次性归位（稳）；`_apply_visible_rules`（分类/搜索过滤）本来就按 `rules` 顺序，不受影响

### 验证
- 打乱 `row_widgets` 后调用重排，顺序恢复为规则顺序
- 同一规则集连续两次真实 GUI 扫描，终态卡片顺序完全一致且等于规则顺序
- `selftest` / `test_regress` / `smoke_gui_test` 全部通过

## v4.2.0（2026-09-12）多核并行 + 感知流畅度（卡顿优化）

### 用户反馈
"总感觉用起来很卡不丝滑，能否通过增加动画变得不卡，或者多调用系统核心"

### 澄清
- **加动画不会让软件变快**——动画本身也要占用主线程，做多了反而更卡。
- 真正的"丝滑" = 后台多核干活 + 主线程减负 + 廉价感知反馈。本版按此组合优化。

### 真快（多核 + 少 syscall）
- **并行扫描**：`engine.scan_rules_parallel()`，path 类规则全并行（workers = min(16, CPU数)，本机 16 线程），SSD 上 30+ 条规则提速数倍；小规则不再被 deep/update 大遍历阻塞
- **快速遍历**：`os.walk + getsize` 改为栈式 `os.scandir`，`DirEntry` 一次拿到类型+大小，syscall 减半；deep/update 剪枝改为"不入栈"（整棵子树直接跳过）
- **并行删除**：永久删除模式大批量（≥500 文件）分块多线程删（workers ≤ 8），小批量沿用串行（避免线程池开销）；回收站模式保持串行（SHFileOperation 按目录批量的语义要求）
- **线程安全重构**：后台线程绝不直接碰 Tk（原来 `after` 跨线程在非 mainloop 下会 `RuntimeError` 卡死），一律 `queue.Queue` + 主线程 60ms 轮询；新老扫描/清理各有代际计数防串台

### 看着丝滑（廉价感知动画，主线程单卡 <0.3ms）
- **骨架屏**：点扫描瞬间即出 6 张占位 + 单一定时器呼吸（380ms 一 tick），不再"点击后冻住"
- **确定性进度**：扫描/清理进度按完成数推进（留 5% 给收尾建卡），替代纯"流动条"，等待不再焦虑
- **分批建卡**：完成回调合并为每帧最多 6 张（~50ms 一帧），40+ 张卡不再一次性淹没主线程，进度动画与滚动始终有时间片
- **入场呼吸描边**：新卡按序错开 0/28/56ms… 做 2 次描边闪光（纯 `itemconfig`，无 configure 级联），波浪式浮现
- **忙时降级**：扫描/清理/建卡期间 hover 与按钮动画降级为瞬时变色（无 after 链），结束自动恢复；图标加载由"每卡一线程"收敛为 2 线程共享池

### 验证
- `selftest` / `test_regress` / `smoke_gui_test` 全部通过
- GUI 实测：10 规则 × 30 文件扫描完成建卡 10 张、进度到顶、骨架撤除；取消路径保留部分结果正常结束
